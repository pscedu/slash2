...
<xdc
	...
	<oof:p>
		IOS Load Balancing: How to choose an import for a given read/write
		operation.
	</oof:p>
	<oof:p>Two conflicting phenomena must be managed:</oof:p>
	<oof:list>
		<oof:list-item>Exploiting server-side caches</oof:list-item>
		<oof:list-item>Not flooding a single server with requests</oof:list-item>
	</oof:list>
	<oof:p>
	...
	<oof:header size="1">Size range (very small to very large)</oof:header>
	<oof:p>
	...
	<oof:header size="1">Read-before-write: Client-side buffering mechanism</oof:header>
	<oof:p>
		...
		independently of the rest of offtree_memb iov.
		This is for dealing with writes which start (or end) within a SLASH2
		slab page.
		The code was very ugly because there is no storage for tracking
		inflight state of a slab block - only an iov.
		Worse yet it was determined that for writes which start in the
		middle of a large iov (lots of blocks) no method exists for
		informing other cache threads that the ios is only partially valid.
	</oof:p>
	<oof:p>
		With this in mind, I've decided to prefetch the entire iovec -
		regardless of its size - when handling read-before-write.
		This should serve fine for a prototype.
		Since the client cache is complicated, the logic for this be handled
		in the initial offtree allocation.
		For instance, unaligned overwrites may be detected in the first call
		into offtree which makes possible the ability to allocate iov's
		accordingly.
	</oof:p>
	<oof:p>
		...
		For large iov's which have already been allocated the situation is
		more grim, though if we wanted, the large iov's could be forcibly
		split across children.
	</oof:p>

	<oof:header size="1">Dealing with parallel I/O servers: What are the
		issues involved?</oof:header>
	<oof:p>
...
resource bessemer {
	...
	<oof:p>
		...
		Now since the backing filesystem is coherent, there is no need to
		bump the generation number again.
	...
	<oof:p>
		...
		denoting that first access ops have already been handled.
		Of course this only applies when peer systems are connected via a
		coherent backing fs.
		In addition, IOS's in coherent environments must always flush their
		slash buffers to the fs.
	</oof:p>
	<oof:p>
	...
	<oof:header size='1'>Design Fallouts:</oof:header>
	<oof:p>
