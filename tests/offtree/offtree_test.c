...
{
	fprintf(stderr,
	    "usage: %s [-B nblksPerReq] [-b slCacheBlkSz] [-D treeDepth] [-d slbFreeDef]\n"
	    "\t[-i iterScheme] [-i iterations] [-m slbFreeMax] [-n slCacheNblks]\n"
	    "\t[-O overLapBlks] [-s startOffset] [-W treeWidth] [-z mapSize (MB)]\n"
	    progname);
	exit(1);
}
...
mygetopt(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "B:b:D:d:I:i:m:n:O:W:z:")) != -1)
		switch (c) {
		case 'B':
...
calc_offset(off_t o)
{
	...
	fprintf(stderr, "\tCalcOffset %"PRIx64"\n", o);
...
process_blk(void *b, off_t o, int rw)
{
	...
	for (i=0; i < nwords; i++) {
		...
		if (rw)
			...
			if (p[i] != bnum) {
				fprintf(stderr, "Err bnum=%"PRIx64" but p[%zu]\n",
					bnum, p[i]);
				return 1;
			}
		}
		...
		for (j=0; j < v->oftiov_nblks; j++) {
			if (o > (v->oftiov_off + (j*slCacheBlkSz)))
				...
				if (OFT_REQ_ENDOFF(req) < (v->oftiov_off + (j*slCacheBlkSz)))
					...
					fprintf(stderr, "iov=%p buf=%p voff=%"PRIx64" roff=%"PRIx64
...
int main(int argc, char **argv)
{
	...
	for (req.oftrq_off=soffa, i=0; i < iterations; i++) {
		fprintf(stderr, "Read Offset %"PRIx64"\n", req.oftrq_off);
		req.oftrq_root   = oftr;
		req.oftrq_memb   = &oftr->oftr_memb;
		req.oftrq_nblks  = nblksPerReq;
		req.oftrq_width  = req.oftrq_depth = 0;
