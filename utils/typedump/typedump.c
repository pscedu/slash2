...
#define PRVALX(val)	pr(#val, (unsigned long)(val), 1)
	...
	PRTYPE(struct slc_wkdata_dircache);
	...
	PRVAL(BMAPGETF_NODIO);
	PRVAL(BMAPGETF_NODISKREAD);
	PRVAL(BMAPGETF_NONBLOCK);
	PRVAL(BMAPGETF_NORETRIEVE);
	PRVAL(BMAP_CACHE_MAX);
	PRVAL(BMAP_CLI_DIOWAIT);
	PRVAL(BMAP_CLI_DIOWAIT_MAX);
	...
	PRVAL(BMPCEF_EIO);
	PRVAL(BMPCEF_FAULTING);
	PRVAL(BMPCEF_FREED);
	PRVAL(BMPCEF_IDLE);
	PRVAL(BMPCEF_PINNED);
	PRVAL(BMPCEF_READAHEAD);
	PRVAL(BMPCEF_READALC);
	PRVAL(BMPCEF_REAPED);
	PRVAL(BMPCEF_TOFREE);
	PRVAL(BPHXC);
	...
	PRVAL(CSVCF_DISCONNECTING);
	PRVAL(CSVCF_FLAGSHIFT);
	...
	PRVAL(SLASH_FID_MDSID_BITS);
	PRVAL(SLASH_FID_MDSID_SHFT);
	PRVAL(SLASH_FSID);
	...
	PRVAL(SLRPC_DISCONNF_HIGHLEVEL);
	PRVAL(SLRPC_MSGADJ);
	PRVAL(SLVRF_ACCESSED);
	PRVAL(SLVRF_CRCDIRTY);
	PRVAL(SLVRF_DATAERR);
	PRVAL(SLVRF_DATARDY);
	PRVAL(SLVRF_FAULTING);
	PRVAL(SLVRF_FREEING);
	PRVAL(SLVRF_LRU);
	...
	PRVAL(SL_FIDC_REAPF_EXPIRED);
	PRVAL(SL_FIDC_REAPF_ROOT);
