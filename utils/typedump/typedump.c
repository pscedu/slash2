...
#define PRVALX(val)	pr(#val, (unsigned long)(val), 1)
	...
	PRVAL(SLC_OPST_READDIR_DONE);
	PRVAL(SLC_OPST_READDIR_RETRY);
	...
	PRVAL(SLI_OPST_HANDLE_REPLREAD_INSERT);
	...
	PRVAL(SLI_OPST_RECLAIM_FILE_FAIL);
	...
	PRVAL(SLM_OPST_RECLAIM_RPC_SKIP);
