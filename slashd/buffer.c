...
sl_slab_reap(int nblks, token_t *tok) {
	...
	nextlru:
		...
		psclist_for_each_entry(r, &b->slb_iov_list, slbir_lentry) {
			...
			/* Otherwise the slb shouldn't be on the LRU */
			...
			ATTR_SET(m->oft_flags, OFT_REAP);			
			sl_oftiov_bfree(m->norl.oft_iov);
		}
