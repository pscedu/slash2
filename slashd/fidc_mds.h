...
static inline void
fmi_init(struct fcoo_mds_info *mdsi, struct fidc_membh *fcmh, void *finfo)
{
	SPLAY_INIT(&mdsi->fmi_exports);
	atomic_set(&mdsi->fmi_ref, 0);
	mdsi->fmi_data = finfo;
...
int mds_fcmh_load_fmi(struct fidc_membh *, void *, int);
