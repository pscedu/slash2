...
int
wipefs_user(const char *fn, const struct stat *stb, int info, int level,
    __unusedx void *arg)
{
	const char *p;
	int rc = 0;
	...
	if (S_ISDIR(stb->st_mode)) {
		...
		else if (info == FTS_DP && rmdir(fn) == -1)
...
int
wipefs_fidns(const char *fn, const struct stat *stb, int info, int level,
    __unusedx void *arg)
{
