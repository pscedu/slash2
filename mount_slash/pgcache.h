...
#define BMPCE_RBWPAGE		(1 <<  6)	/* 0x00040 */
#define BMPCE_EIO		(1 <<  7)	/* 0x00080: I/O error */
#define BMPCE_READA		(1 <<  8)	/* 0x00100: read-ahead */
#define BMPCE_AIOWAIT		(1 <<  9)	/* 0x00200: wait on async read */
...
#define DEBUG_BMPCE(level, b, fmt, ...)					\
	psclogs((level), SLSS_BMAP,					\
	    "bmpce@%p fl=%u:%s%s%s%s%s%s%s%s%s%s "			\
	    "o=%#x b=%p "						\
	    "ts="PSCPRI_TIMESPEC" "					\
	    "ref=%u "							\
	    "owner=%p : " fmt,						\
