...
//	case -ETIMEDOUT:
	...
	if (retry) {
		if (mfh->mfh_retries < 10)
			...
			usleep(1000000);
		OPSTAT_INCR("offline_retry");
	} else
		OPSTAT_INCR("offline_no_retry");
...
bmap_flushable(struct bmapc_memb *b)
{
	...
	if (flush && !force) {
		...
		PFL_GETTIMESPEC(&ts);
		if ((ts.tv_sec + BMAP_CLI_EXTREQSECS < bmap_2_bci(b)->bci_etime.tv_sec) ||
	...
	for (idx = 0; idx + *indexp < psc_dynarray_len(biorqs);
		...
		if (curr->biorq_off <= biorq_voff_get(last)) {
			...
			if (sz > 0) {
				...
				OPSTAT_INCR("bmap_flush_coalesce_contig");
			}
			/*
			 ...
			 * now.
			 */
			OPSTAT_INCR("bmap_flush_coalesce_expire");
			break;
			...
			bwc->bwc_soff = curr->biorq_off;
			psc_dynarray_add(&bwc->bwc_biorqs, curr);
			OPSTAT_INCR("bmap_flush_coalesce_restart");
		}
	}
