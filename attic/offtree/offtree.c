/* $Id: offtree.c 8571 2009-10-23 17:37:21Z yanovich $ */
...
/*
 * offtree_blks_get - allocate memory blocks for request 'req'.  Take
 *	into account an existing memory buffer and manage the allocation
 *	array.  On successful completion, the req->oftrq_darray will
 *	contain an array of iov's which may be used for I/O.  If the
 *	'have_buffer' exists, then it will be placed at the correct
 *	logical location within the iov array.  Since our blocks are
 *	aligned this is possible.
...
/**
 * offtree_region_preprw - Given a memory region request (offtree_req),
 *	ensure that the proper memory allocation is performed to satisfy
 *	the request.  Return an array of iovec buffers in
 *	req->oftrq_darray.  This function is the first stage of a
 *	two-stage recursive process.  Recursion in stage1 is descent
 *	based, where the process recurses until the request's root node
 *	is located.  Several factors are taken into consideration
 *	including existing tree nodes and buffers.  The procedure is
 *	complicated by the fact that offtree makes use of sparseness
 *	where possible.
 * @req: offtree request which contains root and member pointers.
 * Notes: returns the offtree_memb which is the head of the request.
 *	The offtree_memb is tagged with OFT_REQPNDG so that it will not
 *	be freed.  Request offset must be aligned.
 * Notes1: called before processing a system read or write routine to
 *	prepare the file-wise cache for the operation.
 * Return: '0' for success, '-1' failure.  On success the darray pointer
 *	contained in the request will contain the src/dst iovectors, in
